...
def are_all_elements_zeros(dictionary):
    zeros_tensor = torch.zeros(64)
    for value in dictionary.values():
        if not torch.equal(value, zeros_tensor):
            return False
    return True
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    _len_query = {}
    for i1 in range(80):
        query_dict[i1] = [] #torch.zeros(64).cuda()
        _len_query[i1] = 0
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        with autocast(enabled=amp):
            ...
            myloss = 0.0
            output, query_feat, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                                            cat_idx=subcls)
            ...
            if use_my_loss:
                if i==0: #判断query_dict是否全部是空值(从文心一言抄的)
                    for i1 in range(len(class_chosen)): #取出每一个batch
                        query_prototype = pool(query_feat[i]).squeeze() #pool后变成[64,1,1]然后去除1的维度
                        query_dict[class_chosen.cpu()[i1].item()].append(query_prototype) #这里是[64,80,80]
                        _len_query[class_chosen.cpu()[i1].item()] += 1
                else:
                    for i1 in range(len(class_chosen)):  # 应对batch_size ≠ 1的情况
                        now_class = class_chosen[i1].item()
                        now_prototype = pool(query_feat[i1]).squeeze() #query_feat[i]
                        ...
                        for j in (range(80)):  # 对于80个类别计算各自的距离的平均值作为loss,在后续的处理中再区分相同类别和不同类别
                            ...
                            myloss_class[j] = diff_loss #对于每个类别分别进行距离计算

                        myloss += 2 * myloss_class[now_class] - sum(myloss_class) #相同的类别为正,不同的类别为负数
                    for i1 in range(len(class_chosen)):
                        query_prototype = pool(query_feat[i1]).squeeze()  # pool后变成[64,1,1]然后去除1的维度
                        now_class = class_chosen[i1].item()
                        query_dict[now_class].append(query_prototype)  # 这里是[64,80,80]
                        # query_dict[now_class] =  query_dict[now_class] / (_len_query[now_class]+1) * _len_query[now_class]\
                        #                          + query_prototype / (_len_query[now_class]+1)
                        _len_query[class_chosen.cpu()[i1].item()] += 1
                        #计算完loss后将当前的prototype放入到待定prototype中去
