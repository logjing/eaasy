...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        #-----------------------------------text--------------------------------------------
        self.mix_text = nn.Sequential(
            nn.Conv2d(512+64, 256, kernel_size=1,padding=0,bias=False),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, y_m=None, y_b=None, s_x=None, s_y=None, cat_idx=None):
        ...
        supp_feat = self.down_supp(supp_feat) #[1,256,80,80]
        supp_feat_bin = Weighted_GAP(supp_feat, \
                                     F.interpolate(mask, size=(supp_feat_3.size(2), supp_feat_3.size(3)),
                                                   mode='bilinear', align_corners=True)) #[1,256,1,1]
        supp_feat_bin = supp_feat_bin.repeat(1, 1, supp_feat.shape[-2], supp_feat.shape[-1]) #[1,256,80,80]
        supp_feat_item = eval('supp_feat_' + self.low_fea_id) #[1,512,80,80]
        supp_feat_item = rearrange(supp_feat_item, "(b n) c h w -> b n c h w", n=self.shot) #[1,1,512,80,80]
        ...
        # ------------------------------------创建text-feature-------------------------------------------
        # text_feat = text_feature_generation(cat_idx, supp_feat, layer=self.mix_text)
        init_text_feat = self.embeddings[cat_idx]  # [2,512]
        init_text_feat = init_text_feat.unsqueeze(2).unsqueeze(3).repeat(1, 1, 80, 80).cuda()
        fused_feat = torch.cat([init_text_feat, supp_feat], dim=1)
        text_feat = self.mix_text(fused_feat)

        meta_out, weights = self.transformer(query_feat, query_feat_transform, mask, text_feat)
        #sim:[bs,2,80,80]
        # eta_out, weights = self.transformer(query_feat, query_feat_transform, mask, similarity)
