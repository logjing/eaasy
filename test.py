...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_resnet50_manet.yaml', help='config file') # coco/coco_split0_resnet50.yaml
...
def save_tensor(tensor, index, save_dir = r"E:\project\insight1\HDMNet\exp\coco\HDMNet\split0\resnet50\official_result"):
    tensor = tensor.cpu()
    data = tensor.numpy()[0]#.transpose(1, 2, 0)
    image = Image.fromarray(np.uint8(data * 255))
    image.save(os.path.join(save_dir, str(index) + ".png"))

def add_img(tensor1, tensor2, index, save_dir = r"E:\project\insight1\HDMNet\exp\coco\HDMNet\split0\resnet50\official_result"):
    # 假设tensor1是形状为[1, 3, 633, 633]的彩色图片张量
    # 假设tensor2是形状为[1, 633, 633]的灰度图片张量
    tensor1 = tensor1.cpu()
    tensor2 = tensor2.cpu()
    tensor2_rgb = tensor2.repeat(1, 3, 1, 1)
    tensor1 = tensor1.float()
    tensor2_rgb = tensor2_rgb.float()
    result = tensor1/2.0 + tensor2_rgb/2.0
    result_np = result.squeeze().numpy()
    result_img = Image.fromarray(np.uint8(result_np * 255))
    result_img.save(os.path.join(save_dir, str(index) + ".png"))
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(1):
        ...
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            subcls = subcls[0]
            pic_name = loaded_list[i][0].split("_")[-1].replace(".jpg", ".png")
            ...
            # 保存灰度图
            heat_map = softmax(output.detach().cpu(), 1)
            heat_map = heat_map[0, 1, :, :] * 255
            # heat_map[heat_map < 100] = 0
            heat_name = os.path.join(r"C:\Users\Administrator\Desktop\result\pascal\my_output", pic_name)
            cv2.imwrite(heat_name, heat_map)  # 保存灰度图
            ...
            # npy[npy!=255] /= (Max - Min)
            # npy[npy==255] = 0
            # npy_path = "E:\\project\\insight1\\HDMNet\\exp\\coco\\HDMNet\\split0\\resnet50\pred_numpy\\" + str(i) + ".npy"
            # np.save(npy_path ,npy.cpu().detach().numpy())

            output = output.max(1)[1] # (1, 633, 633)
            meta_out = meta_out.max(1)[1]
            base_out = base_out.max(1)[1]

            # heat_name = os.path.join(r"C:\Users\Administrator\Desktop\result\HDMout\onehot", pic_name)
            # cv2.imwrite(heat_name, output[0].detach().cpu().numpy()*255)  # 保存灰度图

            # ori_img = cv2.imread(val_data.data_list[i][0])
            # ori_img = torch.tensor(ori_img).permute(2,0,1).unsqueeze(0)
            # ori_img = map_to_square(F.interpolate(ori_img/255.0, size=(H, W), mode='bilinear', align_corners=True))
            # output_rgb = output.repeat(1, 3, 1, 1) * 255
            # ori_img = ori_img.int()
            # output = ori_img / 2.0 + output_rgb / 2.0
            # add_img(ori_img/255.0, output, str(subcls) + "class_" + str(i))

            # ori_path = val_data.data_list[i][0]
            # subcls = subcls[0].cpu().numpy()[0]
            # ori_img = cv2.imread(ori_path)
            #
            # #save_tensor(output, ori_path + "&" + str(subcls) + "class_" + str(i))
            # save_tensor(output, index = str(i))
            # print(str(i), ori_path.split("/")[-1])
            #
            # if(i>500):
            #     import sys
            #     sys.exit()
...
def map_to_square(x):
    H, W = x.shape[-2:]
    #backmask = cv2.resize(x[0], (H, W), interpolation=cv2.INTER_LINEAR)
    longerside = max(H, W)
