...
from model import hdmnet_chenloss_queryproto_text
from util import dataset
from util import transform, transform_tri, config
from util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='hdmnet_chenloss_queryproto_text')  #
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        with autocast(enabled=amp):
            ...
            if len(out_list) != 3000 or epoch <= 2:
                ...
                output[output != 0] = 1
                area = mask_last_epoch.shape[0] * mask_last_epoch.shape[1]
                last_epoch_loss = torch.sum((mask_last_epoch - output) ** 2)*1.0/area
                out_list[i] = output
                # 根据mask_last_epoch 和 output求last_epoch_loss


            loss = main_loss + args.aux_weight1 * aux_loss1 + args.aux_weight2 * aux_loss2 + last_epoch_loss
            if(torch.isnan(loss)):
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            if ((i + 1) % round((test_num / 10)) == 0) and main_process():
                logger.info('Test: [{}/{}] '
                            'Data {data_time.val:.3f} ({data_time.avg:.3f}) '
                            'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '
