...
for sp in [0,1,2,3]:
    for mm in ['train','val']:
        ...
        root_path = 'E:\project\insight1\MIANet\data\VOC2012'
        ...
        # get data_list
        fss_list_root = 'E:/project/insight1/HDMNet/lists/{}/fss_list/{}/'.format(args.data_set, args.mode)
        fss_data_list_path = fss_list_root + 'data_list_{}.txt'.format(args.split)
        with open(fss_data_list_path, 'r') as f:
            f_str = f.readlines()
        data_list = []
        for line in f_str:
            line = line.replace("ShangJinlong/P","p").replace("/VOCdevkit2012","").replace("MIANet-main","MIANet")
            img, mask = line.split(' ')
